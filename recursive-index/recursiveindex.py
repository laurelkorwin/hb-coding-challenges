...
def recursive_index(needle, haystack):
    ...
    Do this with recursion. You MAY NOT USE A `for` OR `while` LOOP.
    """
    def _recursive_index(needle, haystack, starting_pt):

        if starting_pt == len(haystack):
            return None

        if haystack[starting_pt] == needle:
            return starting_pt

        return _recursive_index(needle, haystack, starting_pt + 1)
